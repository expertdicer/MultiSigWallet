...
async function main() {  
    ...
    // connect address 0 vs address 6
    while(true) {
      try {
        console.log("start create tx connect address!!");
        const connectAddress = await multiSigFactory.updaterConnectAddress(users[0].pubkey, [users[6].pubkey],  {gasLimit: 2000000})
      } catch (e) {
        console.log(e);
      }

      const checkSame = await multiSigFactory.checkSameUser([users[0].pubkey, users[6].pubkey]);
      if (checkSame) {
        console.log("Connect two address", users[0].pubkey, users[6].pubkey);
        console.log("after connect, all address of wallet", await multiSigFactory.getAllAddress(users[0].pubkey));
        break;
      } 
    }
